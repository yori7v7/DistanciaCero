...
- Private manifest generated by user outside repo: yes, result recorded
  separately.
- Manifest included in repo: no.
- Private manifest read by Codex: no.
- Script created: yes, with sanitized fixtures and later audit.
- Insert executed: no.
- Supabase touched: no.
- Storage touched: no.
- Production-ready: no.
...
generator for sanitized fixtures only. S4.6.5.19 records only the sanitized
result of a private manifest generated manually by the user outside the repo.
This document still does not include the real private manifest, does not read
...
- No private manifest read by Codex.
- No insert.
- No Supabase.
- No Dashboard or CLI.
- No Storage.
