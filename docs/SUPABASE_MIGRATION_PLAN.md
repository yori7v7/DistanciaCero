...
- Overrides.
- Hidden.
- Cartas legacy monthly/openWhen existentes que no hayan sido re-guardadas tras
  el fix local de metadata.
- Opened/read.
- Simulation unlocked.
...
- S4.6.4.48 crea `scripts/migration/validate-mock-snapshot.mjs`,
  `scripts/migration/README.md` y fixtures mock `PASS`, `CHECK` y `NO-GO`. El
...
- S4.6.4.52 crea `scripts/migration/run-mock-migration-checks.mjs` como runner
  mock-only para validar exit codes esperados del validador y del dry-run mock.
...
- S4.6.5.11 crea `docs/supabase/PRIVATE_DRY_RUN_NORMALIZER_DESIGN.md` para
  documentar el diseno del futuro normalizador/dry-run privado local-only. El
  documento parte del export UI v2 ya validado, define normalizacion por
...
- S4.6.5.15 documenta
  ...
  docs-only para un futuro primer insert privado en lab desechable. Recomienda
