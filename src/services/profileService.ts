...
const _displayNameCache: Record<string, string> = {}

export function getProfiles(): LocalUser[] {
  return LOCAL_USERS
}

export function getProfileById(id: string): LocalUser | null {
  ...
  if (_displayNameCache[id]) {
    return { id, slug: '', displayName: _displayNameCache[id], role: 'member', avatar: null }
  }
